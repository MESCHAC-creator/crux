...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _joinById(String id) async {
    ...
    String raw = id.trim().toUpperCase();
    if (raw.startsWith('CRUX://JOIN/')) {
      raw = raw.substring('CRUX://JOIN/'.length);
    } else if (raw.contains('/JOIN/')) {
      raw = raw.split('/JOIN/').last;
    }
    final cleanId = raw.split('?').first;
    final lang = context.read<LocaleProvider>().locale.languageCode;

    // First try: direct meeting ID lookup
    var meeting = await _meetingService.getMeetingOnce(cleanId);
    
    // Fallback: try as 6-digit code
    if (meeting == null && cleanId.length == 6 && RegExp(r'^\d{6}$').hasMatch(cleanId)) {
      final meetingId = await _meetingService.getMeetingIdByCode(cleanId);
      if (meetingId != null) {
        meeting = await _meetingService.getMeetingOnce(meetingId);
      }
    }
            ...
            builder: (ctx2, setSheet) => Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    child: GestureDetector(
                      ...
                      child: AnimatedContainer(
                        ...
                        child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                          ...
                          const SizedBox(width: 5),
                          Text('Grande (100+)', style: GoogleFonts.poppins(
                            fontSize: 11, fontWeight: FontWeight.w600,
                            color: _isLargeConference ? Colors.white : Colors.white38)),
                        ]),
                      ),
              ...
              if (_isLargeConference)
                Padding(
                  ...
                  child: Text('Powered by Jitsi Meet — jusqu\'à 100+ participants',
